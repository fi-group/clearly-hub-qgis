...
class Service:
    ...
    def login(self):
        ...
        """
        self.current_user = self.auth_client.get_authenticated_user(
            interactive=True)
    ...
    def get_datasets_page(self, limit=None, offset=0):
        ...
        """
        return self.dataset_manager.get_datasets_page(
    ...
    def get_public_datasets(self, limit=None, offset=0):
        ...
        """
        return self.dataset_manager.get_public_datasets(
            limit=limit, offset=offset)
    ...
    def get_public_digital_twins(self, datasets=None):
        ...
        """
        return self.digital_twin_manager.get_all_digital_twins(
            datasets=datasets)
    ...
    def load_data(self, dataset_id, resource_url=None,
                  resource_format=None, resource_name=None, add_to_project=True):
        ...
        # Prepare metadata
        # Minimal raw dataset for metadata
        dataset_raw = {"_id": ds_info.get("id")}
        ...
        fmt = normalize_format(
            str(ds_info.get("format") or ""), str(ds_info.get("url") or ""))
        ...
        try:
            if fmt == "wfs":
                layer = load_wfs_layer(
                    url, layer_name, parse_wms_url, get_wfs_typenames, token)
            elif fmt in ("geojson", "json", "gpkg", "shp", "xml"):
                layer = load_vector_layer(url, layer_name)
            elif fmt in ("wms", "wcs"):
                layer = load_raster_layer(
                    url, layer_name, fmt, token, get_wms_layers, parse_wms_url)
            elif fmt == "3dtiles":
