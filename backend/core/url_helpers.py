...
def validate_url(url: str) -> bool:
    ...
    if "tileset.json" in url_lower:
        return True
    if url_lower.endswith(".json") and (
            "3dtiles" in url_lower or "tiles" in url_lower):
        return True
    pattern = re.compile(
        r".*\.(wms|wfs|wcs|xml|geojson|json|3dtiles|bim)(\?.*)?$",
        re.IGNORECASE)
...
def get_wfs_typenames(base_url: str, user_token: str = None):
    ...
    try:
        ...
        if response.status_code != 200:
            print(
                f"Failed to fetch WFS capabilities. Status: {
...
def get_wms_layers(base_url: str, user_token: str = None):
    try:
        ...
        if response.status_code != 200:
            print(
                f"Failed to fetch WMS capabilities. Status: {
                    response.status_code}")
            return []
        root = safe_fromstring(response.content)
        ns = root.tag.split("}")[0].strip("{") if "}" in root.tag else ""
