...
class DatasetManager(BaseManager):
    ...
    def _extract_datasets_payload(self, data):
        ...
        if isinstance(data, dict):
            results = data.get("results") or data.get(
                "datasets") or data.get("items") or []
    ...
    def get_all_datasets(self, page_size=200):
        ...
        try:
            ...
            while True:
                ...
                # Safety stop when API does not provide total and returns a
                # short page.
        ...
        if not self._is_authenticated():
            normalized = [
                d for d in normalized if str(
                    d.get("findability") or "").upper() == "PUBLIC"]
    ...
    def get_datasets_page(self, limit=None, offset=0):
        ...
        try:
            data = self.client.get_datasets(
                limit=limit, offset=offset, user_token=token) or {}
        except APIError as e:
    ...
    def get_dataset_by_id(self, dataset_id):
        ...
        try:
            data = self.client.get_dataset_by_id(
                dataset_id, user_token=self._get_user_token())
        except APIError as e:
            self._log(
    ...
    def get_public_datasets(self, limit=None, offset=0):
        ...
        try:
            data = self.client.get_datasets(
                limit=limit, offset=offset, user_token=None) or {}
        except APIError as e:
