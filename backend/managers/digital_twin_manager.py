...
class DigitalTwinManager(BaseManager):
    ...
    def __init__(self, dataset_manager=None, auth_context=None, cache_ttl=None):
        ...
        self.dataset_manager = dataset_manager or DatasetManager(auth_context=auth_context)
    ...
    def _extract_twin_preview_url(self, twin):
        for key in ("previewImage", "landscapePreviewImage", "previewInfoImage"):
    ...
    def _dataset_stats_by_owner_hub(self, datasets):
        ...
        for dset in datasets or []:
            hub_name = str(dset.get("owner_hub") or dset.get("hub") or "").strip()
            if not hub_name:
                continue
    ...
    def _build_twin_info(self, twin, dataset_stats):
        owner_hub = twin.get("ownerHub") or {}
        owner_hub_name = str(owner_hub.get("name") or "Unknown Hub").strip() or "Unknown Hub"
        parent = owner_hub.get("parent") if isinstance(owner_hub, dict) else None
        parent_name = str((parent or {}).get("name") or "").strip()
        stats = dataset_stats.get(owner_hub_name, {"datasets_count": 0, "formats": set()})
    ...
    def get_all_digital_twins(self, datasets=None):
        ...
        try:
            ...
            dataset_stats = self._dataset_stats_by_owner_hub(datasets_for_stats)
            twins = [self._build_twin_info(twin, dataset_stats) for twin in rows]
    ...
    def load_digital_twin(self, digital_twin_id, load_layer_func):
        ...
        try:
            ...
            self._log(f"Failed to fetch datasets for twin {digital_twin_id}: {str(e)}", "warning")
        ...
        try:
            ...
            twin_match = next(
                (tw for tw in twin_rows if str(tw.get("id") or "").strip() == owner_hub_name),
                None,
            )
            if twin_match:
                owner_hub_name = str(twin_match.get("owner_hub") or owner_hub_name).strip()
        except Exception:
            pass
        ...
        for ds_id in dataset_ids:
            try:
                ...
                self._log(f"Failed to load dataset {ds_id}: {str(e)}", "WARNING")
                failed_ids.append(ds_id)
